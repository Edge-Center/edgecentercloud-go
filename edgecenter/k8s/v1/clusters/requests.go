...
func Versions(c *edgecloud.ServiceClient) pagination.Pager {
	url := versionsURL(c)
	return pagination.NewPager(c, url, func(r pagination.PageResult) pagination.Page {
...
func Upgrade(c *edgecloud.ServiceClient, clusterID string, opts UpgradeOptsBuilder) (r tasks.Result) {
	...
	_, r.Err = c.Post(upgradeURL(c, clusterID), b, &r.Body, &edgecloud.RequestOpts{
		OkCodes: []int{200, 201},
	})
	return
}
...
func ListAll(c *edgecloud.ServiceClient, opts ListOptsBuilder) ([]ClusterListWithPool, error) {
...
func SignCertificate(c *edgecloud.ServiceClient, clusterID string, opts ClusterSignCertificateOptsBuilder) (r ClusterCertificateSignResult) {
	...
	_, r.Err = c.Post(url, b, &r.Body, &edgecloud.RequestOpts{
		OkCodes: []int{200, 201},
	})
	return
}
