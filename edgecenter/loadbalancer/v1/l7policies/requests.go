...
func Replace(c *edgecloud.ServiceClient, policyID string, opts ReplaceOptsBuilder) (r tasks.Result) {
	...
	_, r.Err = c.Put(replaceURL(c, policyID), b, &r.Body, &edgecloud.RequestOpts{OkCodes: []int{http.StatusOK}})
	return
}
...
func ReplaceRule(c *edgecloud.ServiceClient, policyID, ruleID string, opts CreateRuleOptsBuilder) (r tasks.Result) {
	...
	_, r.Err = c.Put(rulesReplaceURL(c, policyID, ruleID), b, &r.Body, &edgecloud.RequestOpts{OkCodes: []int{http.StatusOK}})
	return
}
