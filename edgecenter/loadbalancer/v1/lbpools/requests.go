...
func Update(c *edgecloud.ServiceClient, lbpoolID string, opts UpdateOptsBuilder) (r tasks.Result) {
	b, err := opts.ToLBPoolUpdateMap()
	if err != nil {
		r.Err = err
		return
	}
	_, r.Err = c.Patch(updateURL(c, lbpoolID), b, &r.Body, &edgecloud.RequestOpts{
		OkCodes: []int{200, 201},
	})
	return
}
...
func Delete(c *edgecloud.ServiceClient, lbpoolID string) (r tasks.Result) {
...
func DeleteMember(c *edgecloud.ServiceClient, lbpoolID string, memberID string) (r tasks.Result) {
...
func DeleteHealthMonitor(c *edgecloud.ServiceClient, lbpoolID string) (r DeleteHealthMonitorResult) {
	_, r.Err = c.Delete(healthMonitorURL(c, lbpoolID), &edgecloud.RequestOpts{OkCodes: []int{http.StatusNoContent}})
	return
}
