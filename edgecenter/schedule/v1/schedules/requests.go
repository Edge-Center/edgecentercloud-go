...
func Get(c *edgecloud.ServiceClient, scheduleID string) (r GetResult) {
...
// values provided.
func Update(c *edgecloud.ServiceClient, scheduleID string, opts UpdateOptsBuilder) (r GetResult) {
	b, err := opts.ToScheduleUpdateMap()
	if err != nil {
		r.Err = err
		return
	}
	_, r.Err = c.Patch(resourceURL(c, scheduleID), b, &r.Body, &edgecloud.RequestOpts{
