...
func TestPorts_GetAllowAddressPairs(t *testing.T) {
	setup()
	defer teardown()

	expectedResp := &Port{PortID: testResourceID, AllowedAddressPairs: []PortsAllowedAddressPairs{{IPAddress: "192.168.0.2"}}}
	URL := path.Join(portsBasePathV1, strconv.Itoa(projectID), strconv.Itoa(regionID), testResourceID, portsAllowAddressPairs)
	...
	respActual, resp, err := client.Ports.GetAllowAddressPairs(ctx, testResourceID)
	require.NoError(t, err)
	require.Equal(t, resp.StatusCode, 200)
	require.Equal(t, respActual, expectedResp)
}
